...
def main():
    ...
    '''
    '''
    X, Y = node2vec_link_prediction_feature_extraction.getXYFromEmb(bill_term = 100,fin_start_year = 1985,fin_end_year = 1986)
    X_test, Y_test = node2vec_link_prediction_feature_extraction.getXYFromEmb(bill_term = 101,fin_start_year = 1987,fin_end_year = 1988)
    ...
    for i in range(10,200,10):
        ...
        clf = link_prediction.getlogistic(X_new,Y)
        X_test_new = SelectKBest(chi2, k=i).fit_transform(X, Y).transform(X_test)
        kscores_test.append(clf.score(X_test_new,Y_test))
        kscores_train.append(clf.score(X_new,Y))        
