...
def loadFinancialData(start_year, end_year):
    ...
    term_financial_data = term_financial_data.rename(index=str, columns={"NId": "DstNId", "ComNodeId": "SrcNId"})
    return term_financial_data[['DstNId','SrcNId']]
        ...
        for j in range(i+1,len(legislator_node)):
            if i != j:
                ...
                if result:
                    ...
                    Y.append((legislator_node[i],legislator_node[j],0))
...
def getFeatures(G_CoSponsor, G_Campaign, bill_node, legislator_node, comm_node):
    ...
    for index, row in X.iterrows():
        if G_Campaign.IsNode(row['node_i']) == False or G_Campaign.IsNode(row['node_j']) == False:
            ...
            X['Degree_Diff'][index] = abs(G_Campaign.GetNI(row['node_i']).GetDeg() - G_Campaign.GetNI(row['node_j']).GetDeg())
...
def main():
    ...
    financial_data = financial_data.rename(index=str, columns={"NId": "DstNId", "ComNodeId": "SrcNId"})
    ...
    legislator_node_from_campaign = fin_df['DstNId'].unique()

    G_CoSponsor = getSponsorLink(df)
    G_Campaign = common_function.getGraph(fin_df)
